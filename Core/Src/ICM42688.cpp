...
ICM42688::ICM42688(SPI_HandleTypeDef &bus, GPIO_TypeDef* csPort, uint16_t csPin) {
  _spi = &bus; // SPI bus
  _csPort = csPort; // chip select port
...
int ICM42688::writeRegister(uint8_t subAddress, uint8_t data) {
  ...
  HAL_SPI_Transmit(_spi, &subAddress, 1, 50);
  while(HAL_SPI_GetState(_spi) == HAL_SPI_STATE_BUSY);
  HAL_SPI_Transmit(_spi, &data, 1, 50);
  while(HAL_SPI_GetState(_spi) == HAL_SPI_STATE_BUSY);
  HAL_GPIO_WritePin(_csPort, _csPin, GPIO_PIN_SET);
...
int ICM42688::readRegisters(uint8_t subAddress, uint8_t count, uint8_t* dest) {
  ...
  HAL_SPI_Transmit(_spi, &data, 1, 50);
  while(HAL_SPI_GetState(_spi) == HAL_SPI_STATE_BUSY);
  HAL_SPI_Receive(_spi, dest, count, 50);
